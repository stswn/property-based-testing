...
export const TrackNameArb = fc.hexaString(1, 10);

export const MusicPlayerCommandsArb = fc.commands([
  fc.constant(new PlayCommand()),
  fc.constant(new PauseCommand()),
  fc.constant(new NextCommand()),
  fc.record({ position: fc.nat(100), trackName: TrackNameArb }).map(d => new AddTrackCommand(d.position, d.trackName))
]);
